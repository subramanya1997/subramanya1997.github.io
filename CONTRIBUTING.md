...
Automation-loop submissions are Markdown files in `content/loops/`. Use the form at `/awesome-loops/` or open a pull request that adds one file to `content/loops/`. The form opens a GitHub issue with a generated Markdown payload; `.github/workflows/loop-submission-pr.yml` turns that issue into a reviewable PR.
