...
description: "Shell scripting guide for files and directories: create, copy, move, delete, read, write, and search files using practical Bash examples."
...
image: /assets/images/og/demystifying-the-shell-scripting-working-with-files-and-directories.png
tags: [Shell Scripting, Bash, Shell, File Management, Directory Management]
schema_type: TechArticle
faq:
  - q: "How do I create a file or directory in a shell script?"
    a: "Use `touch new_file.txt` to create an empty file and `mkdir new_directory` to create a directory."
  - q: "How do I copy or delete a directory and its contents?"
    a: "Both `cp` and `rm` need the `-r` recursive option for directories, as in `cp -r source_directory destination_directory` and `rm -r directory_to_delete`."
  - q: "What is the difference between the > and >> operators?"
    a: "The `>` operator overwrites a file with the output, while `>>` appends the output to the end of the existing file."
  - q: "How do I read a file line by line in Bash?"
    a: "Use a `while` loop with `read`, for example `while IFS= read -r line; do echo \"Line: $line\"; done < file_to_read.txt`. Setting `IFS=` and using `-r` preserves whitespace and backslashes."
ready: true
---
